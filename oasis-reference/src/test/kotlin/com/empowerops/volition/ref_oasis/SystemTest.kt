...
class SystemTest {
    ...
    fun create(): OptimizerEndpoint {
        ...
        val evaluationEngine = EvaluationEngine(sharedResources, FixValueOptimizer(5.0), modelService, pluginService, eventBus, logger)
        ...
        val apiService = ApiService(modelService, optimizerService)
    ...
    @Test
    fun `when unregister non exist should stop the call`() {
        //setup
        val optimizerEndpoint = create()
    ...
    @Test
    fun `when send update with no proxy should do the update only the sim`() {
        //setup
        val endpoint = create()
        endpoint.registerRequest(registerDTO, mock())
        val responseObserver = mock<StreamObserver<NodeChangeConfirmDTO>>()
    ...
    @Test
    fun `when send update with proxy should do the update both`() {
        //setup
        val endpoint = create()
        endpoint.registerRequest(registerDTO, mock())
     ...
     */
    @Test @Disabled
    fun `when do a full run loop and stop after 3 iterations`() = runBlocking<Unit> {
        //setup
        val endpoint = create()
...
//        """.trimMargin())
    ...
    @Test
    fun `when error happen during evaluation`() = runBlocking<Unit> {
        //setup
        val endpoint = create()
        val channel = Channel<RequestQueryDTO>()
        val simulation = Simulation(
        ...
        val feedAndBuildResponseList: List<RequestQueryDTO> = channel.feedAndBuildResponseList(listOf(
                ...
                { endpoint.offerErrorResult(errorResponse, mock())
                    runBlocking{
                        sharedResources.iterationFinished.receive()
                    }
                }
        ))
    ...
    fun `when force stopped during evaluation`() = runBlocking<Unit> {
        ...
        val feedAndBuildResponseList: List<RequestQueryDTO> = channel.feedAndBuildResponseList(listOf(
                ...
                { /**First evaluation*/ endpoint.stopOptimization(stopRequest, mock())
                    runBlocking{sharedResources.stopPending!!.await()}
                    optimizerService.forceStop()
                }
        ))

        sharedResources.runLoopFinished!!.await()
